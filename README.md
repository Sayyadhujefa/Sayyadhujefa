<h1 align="center">Hi 👋, I'm Sayyad Hujefa Ayub</h1>
<h3 align="center">Artificial Intelligence & Machine Learning Student | AI Enthusiast | Web Developer</h3>
<p align="center">
<img src="https://komarev.com/ghpvc/?username=Sayyadhujefa&label=Profile%20Views&color=0e75b6&style=flat" />
</p>
<p align="center">
<img src="https://streak-stats.demolab.com?user=Sayyadhujefa&theme=github-dark&hide_border=true"/>
</p>
---

## 🎓 About Me

🎓 Final Year **B.E. Artificial Intelligence & Machine Learning Student**

I am passionate about building **AI-powered systems, robotics applications, and intelligent automation tools**.
My focus is on combining **Artificial Intelligence, Machine Learning, and IoT systems** to solve real-world problems.

* 🔭 Currently working on **ROBEX – Autonomous Robot**
* 🌱 Exploring **AI, Robotics, and Intelligent Automation**
* 💻 Interested in **Machine Learning, Robotics, and Software Development**

---

## 🧠 Technical Skills

**Programming Languages**

* Python
* JavaScript

**Artificial Intelligence & Machine Learning**

* Machine Learning
* Data Analysis
* Data Visualization

**Web Development**

* HTML
* CSS
* JavaScript

**Tools & Technologies**

* Git
* GitHub
* IoT Systems
* Robotics Development

---

## 🤖 Major Projects

### 🧠 IoT-Based Robot Powered by LLM

An intelligent robot integrated with **Large Language Models (LLM)** capable of interacting with users and performing smart tasks.

### 📂 AI-Based Smart File Organizer

An AI system that **automatically organizes files** using machine learning techniques.

### 🚀 ROBEX – Autonomous Robot *(Ongoing Project)*

Developing an autonomous robot capable of **intelligent navigation and task automation**.

---

## 💻 Python & Machine Learning Projects

* AI Chatbot using Python
* Machine Learning Models
* Data Visualization Projects
* Automation and AI Utility Tools

---

## 📊 GitHub Statistics

<p align="center">

<img src="https://github-readme-stats.vercel.app/api?username=YOURUSERNAME&show_icons=true&theme=github_dark"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOURUSERNAME&layout=compact&theme=github_dark"/>

</p>

---

## 📫 Connect With Me

📧 Email: [sayyadhujefa04@gmail.com](mailto:sayyadhujefa04@gmail.com)
💼 LinkedIn: https://linkedin.com/in/SayyadHujefa

---
<img src="https://github-readme-activity-graph.vercel.app/graph?username=Sayyadhujefa&theme=github-dark"/>
⭐ Passionate about **Artificial Intelligence, Robotics, and building intelligent systems.**
